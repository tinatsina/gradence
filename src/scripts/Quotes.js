const warhammer = [
  ...
  {
    text: 'Fear the shadows; despise the night. There are horrors that no man can face and survive.',
...
function getRandomQuote() {
  const idx = Math.floor(Math.random() * warhammer.length);
  const quote = warhammer[idx];
  // Return only the quote text, no citation
  return quote.text;
}
